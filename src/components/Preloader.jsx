...
function calcLayout(vw, vh) {
    const canvasW = vw * 0.7;
    const canvasH = vh * 0.7;

    const isMobile = vw < 600;

    const pct = isMobile ? 0.45 : 0.38;
    const scale = (Math.min(canvasW, canvasH) * pct) / 300;

    const sPx = 300 * scale; // pixel size of the S logo

    const gap = isMobile ? 12 : 20;

    const brandFs = isMobile ? 22 : 50;
    const solarFs = isMobile ? 11 : 14;
    const solarPadV = isMobile ? 3 : 5;
    ...
    let cx, cy;
    let textLeft, textTop, textTransform, textAlign;

    if (isMobile) {
        // ── Mobile: stack logo ABOVE text, both centred on screen ──
        const totalH = sPx + gap + brandFs * 2.5; // rough group height
        const groupTop = (vh - totalH) / 2;

        // S logo centre: horizontally centred on screen, at top of group
        cx = vw / 2;
        cy = groupTop + sPx / 2;

        // Text block sits below the logo
        textLeft = `${vw / 2}px`;
        textTop  = `${groupTop + sPx + gap}px`;
        textTransform = "translateX(-50%)";
        textAlign = "center";
    } else {
        // ── Desktop: logo left, text right, both vertically centred ──
        const textBlockW = brandFs * 7 * 0.72;
        const groupW = sPx + gap + textBlockW;

        cx = canvasOffX + (canvasW - groupW) / 2 + sPx / 2;
        cy = canvasOffY + canvasH / 2;

        textLeft = `${cx + sPx / 2 + gap}px`;
        textTop  = `${cy}px`;
        textTransform = "translateY(-50%)";
        textAlign = "flex-start";
    }

    const glowSize = isMobile ? "42vmin" : "35vmin";
...
export default function SavorkaPreloader({ onComplete }) {
    ...
    const rebuild = useCallback(() => {
        ...
        const step = l.isMobile ? 9 : 7;
        const targets = getFilledPixels(outer, inner, step);
        targets.sort((a, b) =>
            Math.hypot(a.x - 150, a.y - 150) - Math.hypot(b.x - 150, b.y - 150)
        );
        const total = targets.length;

        // cx/cy are absolute screen coords; translate into canvas-local coords
        const localCx = l.cx - l.canvasOffX;
        const localCy = l.cy - l.canvasOffY;

        dotsRef.current = targets.map(
            (t, i) => new OrbitalDot(t.x, t.y, i, total, localCx, localCy, l.scale)
        );
        setLayout(l);
    }, []);
    ...
    const {
        isMobile,
        canvasW, canvasH, canvasOffX, canvasOffY,
        textLeft, textTop, textTransform, textAlign,
        brandFs, solarFs, solarPadV, solarPadH, lineW,
        glowSize, cx, cy, gap,
    } = layout;
        ...
        <AnimatePresence>
            {visible && (
                ...
                >
                    ...
                    {/* Canvas — full viewport, draws everywhere */}
                    <canvas
                        ref={canvasRef}
                        style={{
                            position: "absolute",
                    ...
                    <div
                        style={{
                            ...
                            // On mobile, centre children; on desktop, left-align
                        ...
                        >
                            <motion.div
                                ...
                                style={{
                                    ...
                                    padding: `${solarPadV}px ${solarPadH}px`,
                            ...
                            >
                                <motion.div
                                    style={{
                                        ...
                                        background: "linear-gradient(90deg, #eef71e, #35a28b, #eef71e)",
                                        backgroundSize: "250% auto",
                                    }}
                                    animate={brandVisible ? { backgroundPosition: ["0% center", "250% center"] } : {}}
                                    transition={{ duration: 4, ease: "linear", repeat: Infinity }}
                                />
                                <motion.div
                                    style={{
                                        position: "absolute", inset: 0, borderRadius: 4,
                                        boxShadow: "0 0 16px rgba(120,200,120,0.3)", zIndex: 0,
                                    }}
                                    animate={brandVisible ? { opacity: [0.3, 0.7, 0.3] } : {}}
                                    transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }}
                                />
                                <motion.div
                                    style={{
                                        position: "absolute", top: 0, left: "-40%",
                                        width: "40%", height: "100%", zIndex: 1,
                                        background:
                                            "linear-gradient(120deg, transparent, rgba(255,255,255,0.55), transparent)",
                                        transform: "skewX(-20deg)", pointerEvents: "none",
                                    }}
                                    animate={brandVisible ? { left: ["-40%", "120%"] } : {}}
                                    transition={{ duration: 1.8, ease: "easeInOut", delay: 0.9 }}
                                />
                                <div style={{ display: "flex", gap: isMobile ? 2 : 3, position: "relative", zIndex: 2 }}>
                                    {"SOLAR".split("").map((char, i) => (
                                        <motion.span
                                            key={i}
                                            initial={{ opacity: 0, y: 16 }}
                                            animate={brandVisible ? { opacity: 1, y: [16, -3, 0] } : {}}
                            ...
                            <motion.div
                                ...
                                style={{
                                    ...
                                    background:
                                        "linear-gradient(90deg, rgba(255,255,255,1) 0%, rgba(255,255,255,0.5) 40%, rgba(255,255,255,0.15) 75%, rgba(255,255,255,0) 100%)",
                                }}
                            />
                        </div>
                    </div>
